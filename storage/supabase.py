...
class SupabaseLogger:
    ...
    async def get_workflow_run(self, run_id: str) -> Optional[Dict[str, Any]]:
        """
        Get workflow run data by run_id.
        
        Args:
            run_id: The workflow run ID to retrieve
            
        Returns:
            Workflow run data if found, None otherwise
        """
        try:
            result = self.client.table("workflow_runs").select("*").eq("run_id", run_id).execute()
